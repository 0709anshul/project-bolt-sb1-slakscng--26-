...
export function EditTemplateModal({ template, isOpen, onClose, onSuccess }: EditTemplateModalProps) {
  const { getTemplateItems } = useTaskTemplates();
  const { update, isLoading, error } = useUpdateTemplate();
  const [formData, setFormData] = useState({
    name: template.name,
    description: template.description || '',
    items: [] as TaskTemplateItem[]
  });

  useEffect(() => {
    getTemplateItems(template.id)
      .then(items => setFormData(prev => ({ ...prev, items })))
      .catch(console.error);
  }, [template.id, getTemplateItems]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await update(template.id, formData);
      onSuccess?.();
      onClose();
    } catch (err) {
      // Error handling is managed by useUpdateTemplate
    }
  };
